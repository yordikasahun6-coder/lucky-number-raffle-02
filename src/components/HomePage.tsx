...
export default function HomePage({
  ...
  const footerBadges = [
    {
      icon: null,
      emoji: "✈️",
      title: t("footerTelegramTitle"),
      desc: t("footerTelegramDesc"),
    },
    {
      icon: null,
      emoji: "📩",
      title: t("footerSmsTitle"),
      desc: t("footerSmsDesc"),
    },
    {
      icon: null,
      emoji: "🔒",
      title: t("footerSecureTitle"),
      desc: t("footerSecureDesc"),
    },
    {
      icon: null,
      emoji: "📡",
      title: t("footerLiveDrawTitle"),
      desc: t("footerLiveDrawDesc"),
    },
  ];
  ...
  return (
    <main className="min-h-screen relative overflow-hidden">
      ...
      <header className="max-w-3xl mx-auto px-5 pt-6 relative z-10">
            ...
            <div className="relative">
              {assets.hero_image ? (
                <img
                  ...
                  alt="Lottery"
                  className="float-hero machine-sway max-h-64 object-contain relative"
                />
              ) : (
                <div className="float-hero machine-sway h-56 w-56 rounded-full bg-[#E7F5EC]" />
              )}

              {/* Soft sparkle glint that sweeps across the machine */}
              <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-2xl">
                <div
                  className="sparkle-sweep absolute top-0 h-full w-16 bg-gradient-to-r from-transparent via-white/50 to-transparent"
                  style={{ left: "0%" }}
                />
              </div>

              {/* Twinkling highlights near where the coins sit */}
              <span className="twinkle-1 absolute bottom-6 left-2 text-lg pointer-events-none">
                ✨
              </span>
              <span className="twinkle-2 absolute bottom-2 left-10 text-sm pointer-events-none">
                ✨
              </span>
              <span className="twinkle-3 absolute bottom-10 left-16 text-xs pointer-events-none">
                ✨
              </span>
            </div>
          </div>
      ...
      <Reveal>
        <section className="max-w-3xl mx-auto px-5 pb-8 relative z-10 space-y-3">
          <PrizeShowcase prizes={prizes} />
          <PrizeDisclaimer text={settings.prize_disclaimer} />
        </section>
      </Reveal>
      ...
      <Reveal>
        <section
          id="buy-ticket"
          className="max-w-3xl mx-auto px-5 pb-8 relative z-10 scroll-mt-6"
        >
          <div className="ticket-stub hover-lift px-6 py-6">
            <p className="[font-family:var(--font-mono)] text-[10px] tracking-widest text-[#E0A72E] uppercase mb-1">
              Step 2
      ...
      <footer className="bg-[#0F5132] py-10 relative z-10">
        <div className="max-w-3xl mx-auto px-5 grid grid-cols-2 md:grid-cols-4 gap-6">
          {footerBadges.map((b, i) => (
            <Reveal key={i} delay={i * 100}>
              <div className="flex items-center gap-2.5 justify-center md:justify-start text-center md:text-left flex-col md:flex-row">
                ...
                <div>
                  ...
                  <p className="text-[#B8D4C4] text-[10px] sm:text-xs">
                    {b.desc}
                  </p>
                </div>
              </div>
